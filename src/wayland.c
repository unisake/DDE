...
void focus_toplevel(struct wayland_toplevel *toplevel) {
	...
	dde_server *server = toplevel->server;
...
//キーバインドのイベント
bool Alt_handle_keybinding(dde_server *server, xkb_keysym_t sym) {
	//安全の為、TTYに戻れるようにする。コイツらは設定で弄ることはない。
	switch (sym) {
	case XKB_KEY_Escape:
...
bool Super_handle_keybinding(dde_server *server, xkb_keysym_t sym){
	//設定で登録出来るキーバインドはsuperで修飾されたときのみ。
	switch (sym) {
...
void keyboard_handle_key(
	...
	dde_server *server = keyboard->server;
...
void server_new_keyboard(dde_server *server,
...
void server_new_pointer(dde_server *server,
		struct wlr_input_device *device) {
...
void seat_request_cursor(struct wl_listener *listener, void *data) {
	dde_server *server = wl_container_of(
...
struct wayland_toplevel *desktop_toplevel_at(
		dde_server *server, double lx, double ly,
...
void process_cursor_motion(dde_server *server, uint32_t time) {
...
void server_new_xdg_toplevel(struct wl_listener *listener, void *data) {
	...
	dde_server *server = wl_container_of(listener, server, new_xdg_toplevel);
...
{
    return calloc(1, sizeof(dde_server ));
}
